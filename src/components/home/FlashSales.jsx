...
export default function FlashSales({ products = MOCK_PRODUCTS }) {
  ...
  return (
    <section className="w-full my-6 sm:my-10 p-3 sm:p-7 rounded-2xl sm:rounded-3xl bg-gradient-to-b from-[#180A03] via-[#0E0E0E] to-[#0A0A0A] border border-[#FF5B00]/40 shadow-2xl relative overflow-hidden">
      {/* Üst Başlıq və Geriyə Sayan Taymer */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 pb-4 sm:pb-6 border-b border-[#2A150A]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 bg-[#120703] border border-[#FF5B00]/40 py-1.5 sm:py-2 px-2.5 sm:px-3.5 rounded-xl sm:rounded-2xl self-start sm:self-auto shrink-0">
          <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#FF5B00] animate-spin" style={{ animationDuration: '6s' }} />
          <span className="text-[11px] sm:text-xs font-bold text-neutral-300 mr-1">Bitməsinə:</span>
          <div className="flex items-center gap-1 font-mono font-black text-[11px] sm:text-xs text-white">
            <span className="bg-[#241005] border border-[#FF5B00]/50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg text-[#FF5B00]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-4 md:gap-5 mt-4 sm:mt-6">
        {displayProducts.map((prod, idx) => {
          ...
          return (
            <div
              key={prod.id}
              className="p-2.5 sm:p-4 rounded-2xl bg-[#111111] border border-[#242424] hover:border-[#FF5B00]/60 transition-all flex flex-col justify-between group shadow-lg"
            >
              <div>
                {/* Kliklənən Məhsul Şəkli */}
                <Link
                  href={`/products/${prod.handle}`}
                  className="block relative aspect-square rounded-xl bg-[#181818] overflow-hidden mb-2.5 cursor-pointer group/img"
                ...
                <div className="flex items-center gap-1 text-yellow-400 mb-1">
                  ...
                  <span className="text-[10px] sm:text-[11px] font-bold text-white ml-0.5">4.9</span>
                </div>
              ...
              <div className="mt-3 pt-2.5 border-t border-[#1F1F1F] space-y-2 sm:space-y-2.5">
                {/* Qiymət */}
                ...
                {/* Satış Zolağı (Urgency Scarcity) */}
                <div>
                  <div className="flex justify-between text-[9px] sm:text-[10px] text-neutral-400 font-semibold mb-1">
                    <span>Satıldı: {soldPercentages[idx % soldPercentages.length]}%</span>
                    <span className="text-red-400 hidden xs:inline">Son ədədlər</span>
                  </div>
                ...
                {/* Düymə (Mobil uyğun min 42px - 48px) */}
                <button
