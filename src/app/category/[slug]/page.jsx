...
export default async function CategoryPage({ params }) {
  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen py-6 sm:py-10">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-xs text-neutral-400 mb-6 overflow-x-auto whitespace-nowrap pb-1">
          ...
          <span className="text-neutral-200 font-medium">{categoryTitle}</span>
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-4 border-b border-[#1C1C1C]">
          ...
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-xs font-bold text-neutral-400 hover:text-white transition-colors self-start sm:self-auto"
          >
        ...
        {products.length > 0 ? (
          ...
          <div className="text-center py-16 p-6 rounded-3xl bg-[#0F0F0F] border border-[#222222]">
            <p className="text-neutral-400 text-sm">Bu kateqoriyada hazırda məhsul tapılmadı.</p>
            <Link
              href="/"
